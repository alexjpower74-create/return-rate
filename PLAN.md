...
**Return Rate: point your phone at a container's barcode and get the exact Newfoundland refund for that item, or a plain "the depot doesn't take this one".** One item, one answer, from the MMSB rules, no guessing at bags. Built for customers of APCO Recycling (Grand Falls-Windsor, NL), reached through the QR sheet already on the depot counter; a portfolio showpiece (browser barcode scanning, a rules engine with tests, a crowd-built product list that never invents a refund).

**The rules live in `docs/RULES.md`** (researched from the regulation, the MMSB Distributor Guide chart and the MMSB/NLC FAQs; every line cited). Read it before writing a line of code. The short version: the refund depends on the drink AND the container. 10¢ only for wine and spirits (incl. coolers, ciders, sake) in plastic, glass or tetra/gable, except 50 mL miniatures; every other refundable container is 5¢ (all cans including canned wine and cocktails, all beer, pouches, bag-in-box, all non-alcoholic drinks up to 5 L). Not refundable: milk, fortified plant milks (unless labelled 'not a source of protein'), infant formula, meal replacements and formulated liquid diets, concentrates, distilled water, over 5 L, out-of-province purchases, and local brewers' REFILLABLE beer bottles (those go to the beer store; the app says 'ask at the counter'). Caps off, labels on, don't crush. The app shows the REFUND, never the deposit.

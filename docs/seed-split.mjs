...
const GROUP = {
  milk: 'label-milk',
  'flavoured-milk-beverage': 'label-milk',
  'protein-shake': 'label-milk',
  'plant-milk': 'label-plant',
  'plant-drink-not-protein': 'label-plant',
  'meal-replacement': 'label-nutrition',
  'formulated-liquid-diet': 'label-nutrition',
  'infant-formula': 'label-nutrition',
}
const auto = [],
  counter = []
for (const r of rows) {
  let why = null
  if (LABEL_DEPENDENT.has(r.drink))
    why = 'the label decides (milk, plant, nutrition, coffee-with-milk): the app tells the customer what to read'
  ...
  if (why)
    counter.push({
      ...
      brand: r.brand,
      drink: r.drink,
      label_group: GROUP[r.drink] || 'label-other',
      material: r.material,
      size_ml: r.size_ml,
      why,
    })

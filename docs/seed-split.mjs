...
const GROUP = { milk: 'label-milk', 'flavoured-milk-beverage': 'label-milk', 'protein-shake': 'label-milk', 'plant-milk': 'label-plant', 'plant-drink-not-protein': 'label-plant', 'meal-replacement': 'label-nutrition', 'formulated-liquid-diet': 'label-nutrition', 'infant-formula': 'label-nutrition' };
const auto = [], counter = [];
for (const r of rows) {
  let why = null;
  if (LABEL_DEPENDENT.has(r.drink)) why = 'the label decides (milk, plant, nutrition, coffee-with-milk): the app tells the customer what to read';
  ...
  else if (Number(r.size_ml) > 4000) why = 'near or over the 5 L line: the label test applies';
  else if (r.refillable) why = 'refillable: the label test applies';
  if (why) counter.push({ upc: r.upc, name: r.name, brand: r.brand, drink: r.drink, label_group: GROUP[r.drink] || 'label-other', material: r.material, size_ml: r.size_ml, why });

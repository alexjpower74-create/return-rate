...
for (const c of cases) {
  ...
  try {
    ...
    r = { class: 'THREW', refund_cents: null, why: e.message }
  }
  const gotA = accepted(r),
    gotC = r.class === 'unknown' ? null : r.refund_cents
  const okA = gotA === c.accepted,
    okC = gotC === c.cents,
    okP = c.depot_policy ? r.depot_policy === true : true
  if (!(okA && okC && okP)) {
    bad++
    console.log(

...
const API = (args.api || '').replace(/\/$/, ''); if (!API) { console.error('need --api'); process.exit(2); }
const expected = JSON.parse(readFileSync(new URL('./labels/expected.json', import.meta.url)));
let miss = 0, n = 0;
for (const [file, want] of Object.entries(expected)) {
  n++;
  const fd = new FormData(); fd.append('photo', new Blob([readFileSync(new URL('./labels/' + file, import.meta.url))], { type: 'image/jpeg' }), file);
  const t0 = Date.now();
  ...
  const ok = got === want; if (!ok) miss++;
  console.log(`${ok ? 'ok  ' : 'MISS'} ${String(Date.now() - t0).padStart(5)}ms ${file.padEnd(40)} want=${want} got=${got}  ${(body.why || body.error || '').slice(0, 70)}`);
}

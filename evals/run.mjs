...
if (!api) {
  console.error('Usage: node evals/run.mjs --api <worker url>')
  process.exit(2)
}
...
if (!cases?.length) {
  console.error('No cases in ' + labelsPath)
  process.exit(2)
}
...
for (const c of cases) {
  ...
  try {
    ...
    if (res.status === 200) {
      got = body.class
      cents = body.refund_cents
      name = body.name || ''
    } else if (res.status === 404) {
      got = body.name ? 'ask' : 'unknown'
      detail = body.error || ''
    } else {
      got = `http ${res.status}`
      detail = body.error || ''
  ...
  if (!ok) wrong++
  console.log(
    `${ok ? '  ' : 'X '}${pad(c.upc, 15)}${pad(c.expected, 10)}${pad(got, 10)}${pad(cents, 7)}${name || c.name || ''}${detail ? '  (' + detail + ')' : ''}`,
  )
}

...
const FIELDS =
  'code,product_name,product_name_en,product_name_fr,brands,quantity,product_quantity,product_quantity_unit,categories_tags,packaging_tags,packaging_materials_tags,packaging_shapes_tags,packagings,unique_scans_n,labels_tags,alcohol_100g,nutriments,countries_tags'
...
function toCandidate(p) {
  ...
  if (LIQUOR.has(drink) && /plastic/.test(material || '') && size_ml > 1500)
    reasons.push('wine in "plastic" over 1.5 L: probably a bag-in-box, which pays differently')
  if (size_ml > 5000 && drink !== 'water') reasons.push('over 5 L and not a water jug: probably a case')
  if (drink === 'plant-milk' && /not-a-source-of-protein/.test(labels))
    reasons.push('plant drink labelled not-a-source-of-protein: needs a human look')
  return {
    code,
    name,
    ...
    material,
    refillable: false,
    alcohol_pct: p.alcohol_100g ?? p.nutriments?.alcohol ?? null,
    label_flags: [],
    reasons,
...
async function main() {
  ...
  for (const p of raw) {
    ...
    const r = classify({
      ...
      material: c.material,
      size_ml: c.size_ml,
      refillable: c.refillable,
      alcohol_pct: c.alcohol_pct,
      label_flags: c.label_flags,
    })
    if (!r || !['regular', 'liquor', 'none', 'brewer'].includes(r.class)) {
      drop(`classifier said ${r?.class ?? 'nothing'}`)
      continue
    }
    rows.push({
      upc: c.code,
  ...
  for (const r of rows) {
    ...
    byDrink[r.drink] = (byDrink[r.drink] || 0) + 1
  }
  ...
  if (rows.length < 300) {
    console.error(`Only ${rows.length} rows; target is 300+.`)
    process.exitCode = 1
  }
}

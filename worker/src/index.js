...
const LABEL_GUIDE = {
  'label-milk':
    "This one depends on the label: Milk has no refund, a milk beverage or protein shake does. Take a photo of the front and we'll read it.",
  'label-plant':
    "This one depends on the label: a fortified plant beverage has no refund unless it says 'not a source of protein'. Take a photo of the front and we'll read it.",
  'label-nutrition':
    "This one depends on the label: Meal Replacement, Formulated Liquid Diet and infant formula have no refund. Take a photo of the front and we'll read it.",
...
{"is_beverage": <true only if this container held a ready-to-serve DRINK for people (pop, water, juice, milk, beer, wine, spirits, energy drink, etc.) or a drink concentrate; false for anything else: sauces, condiments, syrups for food, oils, vinegar, medicine, tablets, cleaning products, cosmetics, food, pet products, or no container at all.
   Decide from the CONTAINER and the CONTEXT, not just the words: sauces, marinades, dressings, glazes, ketchup, mustard and barbecue sauce come in squeeze bottles or wide-mouth bottles with flip-top caps, thick opaque contents, serving sizes "per 1 tbsp / 15 mL", and condiment brands (Kraft, Heinz, Sweet Baby Ray's, Bull's-Eye, Diana, Club House, French's, Renée's, Kikkoman, VH, PC/no name sauces). A drink has a screw cap, pull tab or spout, a thin pourable liquid, a volume like 250, 355, 473, 500, 591, 710 mL, 1 L, 1.5 L, 2 L, and words like drink, beverage, juice, water, pop, soda, beer, wine. A flavour name alone (e.g. "Hickory Smoke", "Honey Garlic", "Original") is NOT evidence of a drink. If you cannot tell, is_beverage is false.>,
 "what_it_is": "<two or three plain words for what the product is, e.g. soy sauce, heartburn tablets, apple juice>",
 "name": "<the product name as printed on the front, always filled in when the label is readable>", "brand": "<brand or null>",
...
Treat EVERY container the same way: first decide whether it held a drink at all, from its shape, cap, contents and label, before naming the drink. Use "unknown" only when you truly cannot tell. Do not guess sizes; use the printed volume.
...
async function postLabel(request, env) {
  ...
  const { today } = await env.DB.prepare(
    "SELECT COUNT(*) AS today FROM lookups WHERE upc = 'label' AND created >= strftime('%Y-%m-%dT00:00:00Z','now')",
  ).first()
  if (today >= cap)
    return err('The photo reader has done its share for today. Try the barcode, or look for the words Return for Refund on the label.', 503)
  ...
  try {
    ...
    return err('Please send a photo of the label.', 400)
  }
  ...
  if (!read) {
    return err(
      "We couldn't read the label just now. Try again in better light, or " + LABEL_TEST.charAt(0).toLowerCase() + LABEL_TEST.slice(1),
      503,
    )
  }
  ...
  const evidence = []
  if (read.says_meal_replacement) {
    drink = 'meal-replacement'
    evidence.push('the label says Meal Replacement, Formulated Liquid Diet or infant formula')
  } else if (read.says_milk) {
    drink = 'milk'
    evidence.push('the label calls it Milk')
  } else if (read.says_fortified_plant && read.says_not_source_of_protein) {
    drink = 'plant-drink-not-protein'
    evidence.push('the label says fortified plant-based beverage and not a source of protein')
  } else if (read.says_fortified_plant) {
    drink = 'plant-milk'
    evidence.push('the label says fortified plant-based beverage')
  } else if (drink === 'milk') {
    drink = 'flavoured-milk-beverage'
    evidence.push('it looks like a milk drink but the label does not call it Milk, so it has a deposit')
  } else if (drink === 'protein-shake' || drink === 'flavoured-milk-beverage') {
    evidence.push('it is not labelled as Milk, so it has a deposit')
  } else if (drink === 'plant-milk') {
    evidence.push('a plant-based milk alternative (these are fortified and a source of protein unless the label says otherwise)')
  } else if (drink === 'unknown' && read.is_beverage === true) {
    drink = 'soft-drink'
    evidence.push('a drink that is not milk, a plant milk or a nutrition product')
  }
  ...
  if (!read.readable && drink === 'unknown') {
    return err("We couldn't make out what this is. Take another photo closer, with the front label filling the screen.", 404, {
      ...base,
      accepted: null,
  ...
  if (!read.readable || r.class === 'unknown') {
    ...
    if (
      read.readable &&
      read.says_return_for_refund &&
      drink !== 'unknown' &&
      !['wine', 'spirits', 'sake', 'mead', 'cider', 'cooler', 'seltzer', 'malt-beverage', 'cocktail', 'hard-kombucha'].includes(drink) &&
      (size_ml === null || size_ml <= 5000)
    ) {
      const rr = classify({ drink, material: 'aluminum', size_ml: size_ml || 355 }, env)
      if (rr.class === 'regular')
        return json({
          ...
          refund_cents: rr.refund_cents,
          depot_policy: false,
          why: 'The label says Return for Refund and it is a drink, not milk or a nutrition product: ' + rr.refund_cents + '¢.',
        })
    }
  ...
  return json({
    ...
    refund_cents: r.refund_cents,
    depot_policy: r.depot_policy === true,
    why: r.why + (evidence.length ? ' From the label: ' + evidence.join('; ') + '.' : ''),
  })
}

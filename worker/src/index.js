...
async function postLabel(request, env) {
  ...
  else if (drink === 'milk') { drink = 'flavoured-milk-beverage'; evidence.push("it looks like a milk drink but the label does not call it Milk, so it has a deposit"); }
  else if (drink === 'protein-shake' || drink === 'flavoured-milk-beverage') { evidence.push("it is not labelled as Milk, so it has a deposit"); }

...
if (!API) {
  console.error('Set API=<worker base url>')
  process.exit(2)
}
const arg = (k, d) => {
...
const SEED = {
  upc: arg('upc', process.env.SEED_UPC),
  klass: arg('class', process.env.SEED_CLASS || 'regular'),
...
function check(name, actual, predicate, control) {
  ...
  if (state !== 'ok') red++
  console.log(
    `${state.padEnd(4)} ${name}${state === 'RED' ? `\n     got: ${JSON.stringify(actual)}` : ''}${state === 'VOID' ? ' (negative control also passed, this check measures nothing)' : ''}`,
  )
}

const health = await get('/health')
check('health ok:true with an item count', health, (r) => r.status === 200 && r.body?.ok === true && Number.isInteger(r.body.items), {
  status: 200,
  body: { ok: false },
})
...
if (SEED.upc) {
  ...
  check(`seeded ${SEED.upc} refund is ${SEED.cents}¢`, item, (r) => r.body?.refund_cents === SEED.cents, {
    body: { refund_cents: SEED.cents === 5 ? 10 : 5 },
  })
  check(
    'seeded item has a plain one-sentence why',
    item,
    (r) => typeof r.body?.why === 'string' && /\S/.test(r.body.why) && !/undefined|null|NaN/.test(r.body.why),
    { body: { why: 'undefined' } },
  )
  check('seeded item carries the counter line', item, (r) => r.body?.note === MONEY, { body: { note: 'Guaranteed payout' } })
  check('never the class "unknown" in a 200', item, (r) => r.status !== 200 || r.body?.class !== 'unknown', {
    status: 200,
    body: { class: 'unknown' },
  })
} else {
...
check('unknown never carries a refund', unk, (r) => r.body?.refund_cents === undefined && r.body?.class === undefined, {
  body: { refund_cents: 5 },
})

const bad = await get('/item/12ab')
check('malformed barcode is 400', bad, (r) => r.status === 400, { status: 404 })
check('malformed error is the plain sentence', bad, (r) => r.body?.error === "That doesn't look like a barcode number.", {
  body: { error: 'Invalid UPC' },
})

const stats = await get('/stats')
check(
  'stats has items, lookups_today, unknown_today, top_unknown[]',
  stats,
  (r) =>
    r.status === 200 &&
    ['items', 'lookups_today', 'unknown_today'].every((k) => Number.isInteger(r.body?.[k])) &&
    Array.isArray(r.body?.top_unknown),
